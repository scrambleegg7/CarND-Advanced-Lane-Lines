...
class LaneDetector(object):
    ...
    def combinedBinaryImage(self, image):
        ...
        mask_lane = self.makeBinaryImage(image)
    ...
    def line_detector(self,binary_warped):
        ...
        if self.detected == False:
            ...
            if length_left_lane_ids > 5 and length_right_lane_ids > 5:
                ...
                self.best_left_fit = np.mean(  self.good_curve_left_fit[-30:], axis=0)  
                self.best_right_fit = np.mean(  self.good_curve_right_fit[-30:], axis=0)  
